...
export function buildAskPrompt(question: string, sources: NearestChunk[]): string {
  const sourceBlocks = sources
    .map(
      (source, i) => `<source id="${i + 1}" title="${source.title}">\n${source.content}\n</source>`,
    )
    .join('\n\n');
  return `<sources>\n${sourceBlocks}\n</sources>\n\n<question>\n${question}\n</question>`;
}
  ...
  return {
    async askBlog(question: string): Promise<AskBlogResponse> {
      ...
      try {
        ...
        const sources = (
          await chunkRepository.findNearestChunks(queryEmbedding, MAX_SOURCES)
        ...
        if (sources.length === 0) {
          return { answer: NOT_FOUND_ANSWER, sources: [], answerable: false };
        }
        ...
        if (!output.answerable) {
          return { answer: output.answer, sources: [], answerable: false };
        }
